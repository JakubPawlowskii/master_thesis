...
where \(a\in\{+,-,z\}\), \(\Sp = S^x + i S^y\), \(\Sm = \left(\Sp\right)^{\dagger}\),
 ...
 \end{equation}
From equation~\eqref{eq:spin_op} it is evident, that the Hilbert space our Hamiltonian acts on
is the tensor product of \(L\) copies of single spin spaces \(\mathfrak{h} \cong \CC^2\), 
\(\mathrm{dim_{\CC}}(\mathfrak{h}) = 2\), therefore the total dimension is \(2^L\).
For numerical calculations, we shall use the so-called Ising basis, consisting of eigenstates
of the \(S^z_{\mathrm{tot}} = \sum_{\ell=1}^{L} \Sz_{\ell}\) operator.


Our main interest in the spin transport in this system. To this end, we first notice that
the total magnetization, or \(z\) component of the total spin \(\Sz_{\mathrm{tot}}\)  is a conserved quantity.
...
\end{align}
which is our desired spin current.



\textcolor{red}{Magnetization and momentum sectors}
Before we finish the introductory chapter, let us notice to particulary useful symmetries of the 
long-range anisotropic Heisenberg Hamiltonian.
